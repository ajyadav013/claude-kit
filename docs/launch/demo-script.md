...
| **5** | 0:50–1:00 | Show the refusal line from the historical 0.82 transcript in `run-artifacts/gate-refused.txt`: `FAIL  cannot close 'contract-clear': medium=1 open finding(s) must be resolved first`. Do not show or repeat its superseded `--force` remedy. | "Gate REFUSES to advance: an open Medium needs resolution or the current structured accepted-risk path." |
...
3. **Gate refusal:** `run-artifacts/gate-refused.txt` (historical 0.82 transcript; show only the
   refusal, not its superseded `--force` remedy).
