...
| 7 | Multi-Agent Collaboration | 33 agents; subagents + Agent-Teams worked example in orchestration-patterns | covered (strong) |
...
| 9 | Learning & Adaptation | `rules/agent-memory.md`; `skills/remember`; the `load-learnings` SessionStart hook; **auto-capture** — the **opt-in** selected-provider `capture-learnings` job (default off: SessionEnd · Claude-only SessionStart catch-up for abrupt closes · per-task Stop). Codex capture uses a bounded filtered/redacted dirty Git diff and trusted writer; historical Codex catch-up safely no-ops | covered |
