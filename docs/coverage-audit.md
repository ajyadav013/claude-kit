...
| **Memory hygiene** (P1-1) | **RULE — always-on** | `rules/agent-memory.md` "Memory hygiene" (verify-before-trust, cited selective attachment, CLAUDE.md precedence); `rules/continuity.md` start-of-turn line; `remember` skill staleness check. Capture is **opt-in** (0.76.0): the `capture-learnings` hook runs a selected-provider background classifier. Claude can use transcript/change context; Codex receives only a bounded filtered/redacted dirty Git diff through the trusted coordinator | all profiles (advisory discipline); capture only via the init-time `capture_mode` choice (default: off) |
| **Resume snapshot** (P1-2) | **RULE + MECH** | `rules/continuity.md` schema for `.ckit/state/pipeline-snapshot.json` + reload-not-rerun protocol; written/read by the authoritative lifecycle/managed executor and surfaced to `orchestrator` + `sdlc`; the neutral state dir is created by native pip installs and loaded by `load-continuity.sh` | all profiles (state mechanism + rule) |
