...
1. Treat `the invocation request` as the invocation input. Accept `--kind
...
3. From the project root, invoke exactly one of these forms:
   ...
   ```text
   ckit maker-checker run --kind <auto|code|design|specification> --task '<task>'
   ckit maker-checker run --resume <run-id> [--kind <kind>] [--task '<exact-frozen-task>']
   ```
...
5. If native-worker termination is unconfirmed, inspect it with `ckit pipeline status` and
