...
growing one giant AGENTS.md.

**Equivalent files for other tools:**
- `.cursorrules` or `.cursor/rules/*.md` (Cursor)
- `.windsurfrules` (Windsurf)
- `.github/copilot-instructions.md` (GitHub Copilot)
- `CLAUDE.md` (an alternate model runtime)
...
- **Summarize *from here* before rewinding or compacting** — write done-state to `.ckit/CONTINUITY.md`
  first, so the summary survives the reset (`continuity`).
...
the session UI where the harness supports it — the host's status interface, where available can render tokens-used /
...
- **Progressive disclosure** — don't load everything up front. Expose names + one-line descriptions and
  ...
  `tool-design`). Preserves the baseline attention budget for the task — aim to keep
...
- **Tool-output offloading** — don't dump large tool/command output into context. Keep a few signal
  ...
  rules in `tool-design`). Compress **content-type-aware**, not blindly:
...
1. **Detect repo type & scope.** Decide what kind of codebase this is (service / UI / mobile /
   ...
   rule (`.ckit/rules/<stack>-patterns.md`), not here — read them rather than hardcoding file
