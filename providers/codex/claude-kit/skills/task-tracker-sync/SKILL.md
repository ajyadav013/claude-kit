...
ckit does not assume a tracker. Use whichever **issue-tracker MCP server** is configured for
the project (see `.codex/config.toml` / the catalog): GitHub, Linear, Jira, or another. Discover the
