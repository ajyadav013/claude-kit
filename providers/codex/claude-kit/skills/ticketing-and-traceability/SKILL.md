...
When the pip CLI is installed, `ckit tickets` renders the store as a live chart — one row per
...
the commit graph with each commit's ticket attached, `ckit tickets <PREFIX>-<N>` opens one
ticket's detail with its full work log, and `--watch 5` re-renders while a run is in flight.

For a browser view, `ckit tickets --html` writes a Kanban board to
`.ckit/state/ticket-board.html` and prints a `file://` URL. It is a plain file, not a server — the
...
- **The board opens once, here.** `ckit tickets --open` at ticket creation gives the human a
