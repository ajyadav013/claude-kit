...
- **Review each bundle in isolated context, concurrently.** Give each bundle its own focused context
  ...
  `mandatory-workflow` lane model, within the Brooks's-law caution in
  `tool-design` §7). Context gathered for one bundle is for *understanding only* — a
...
`quality-gates` (§2.5): an uncited finding is a TODO, not a finding.
