...
5. **Vet third-party skills before installing them.** A skill is instructions your agent will follow with your credentials — treat an untrusted one like an untrusted dependency. Before it enters `.agents/skills/`, run the agent-config supply-chain intake in the `dependency-verification` skill (provenance pin, structural read, hidden-content scan).
