...
You are the **entrypoint** to ckit's autonomous software development lifecycle. The request to
handle is:
...
- `AGENTS.md` — the project's rules and the exact build/test/lint commands.
- `mandatory-workflow` — the full phase pipeline and the defect loop.
- `quality-gates` — ordinary PASS requires zero Critical/High/Medium; Medium can
  only use the distinct structured accepted-risk resolution; plus blind review/Devil's Advocate.
- `rarv-cycle` — the Reason → Act → Reflect → Verify self-check every agent runs.
- `wave-orchestration` — the program-mode contract (audit manifest, risk-ordered
  waves, disjoint boundaries, inventory approval) — required whenever the work classifies as
  program-scale.

Then read `.ckit/CONTINUITY.md` (the `load-continuity` SessionStart hook has already printed it into
context). **Detect an in-progress run:** if **Current Phase** is not idle and **Active Tasks** names a
run matching `the invocation request`, an earlier pipeline is in flight. Prefer the **schema-v2 structured
snapshot** at `.ckit/state/pipeline-snapshot.json`: run `ckit pipeline resume` and use its
ordered ledger/status as the precise resume index (schema in `continuity`). The
...
Read `.ckit/config/stack-catalog.snapshot.yaml`. Its `gates:` and `agents:` lists are the
**authoritative set** of what the installed profile activated. Also read
`.ckit/config/init-options.json` for the stack `selection` (frontend / backend / database) so you
...
- If either authoritative file is absent or unreadable, stop and require a supported
  `ckit` CLI install/upgrade. Do not guess a profile, gate set, or stack selection.
...
```text
ckit pipeline run --provider codex \
...
1. **Classify** the work — bug fix vs. feature; single-stream vs. parallel lanes (backend/frontend);
   ...
   closeout wave. (Where the session has a native dynamic-workflow facility, where supported
   ...
   a full run route through the reduced chain per `risk-classification` →
   Story-level routing.
2. **Record** the plan/context in `.ckit/CONTINUITY.md`, but create and mutate gate-precise state
...
3. **Route skills and models explicitly — and announce fan-out before spawning it.** Every agent
   ...
   Routing table; only skills actually installed under `.agents/skills/`) and runs on the model
   tier its work deserves (`model-tiers`) — cheap for audits/scans, standard for
   ...
   lane/agent count and model tiers in chat and records them in `.ckit/CONTINUITY.md`, so the
...
`mandatory-workflow` §1f) and the run is not program-scale (Mode E), parallelize
across context windows instead of inside one: spawn **one orchestrator per group**, each in its
**run-owned git worktree** per `continuity` → Concurrency. Worktrees contain code and
stage artifacts only: they never receive an independent continuity file, pipeline snapshot, or
gate ledger. The primary checkout's single `.ckit/state/pipeline-snapshot.json` remains authoritative for
...
`.ckit/CONTINUITY.md` reflects the final state, and promote any durable lessons with the
`remember` skill (into `.ckit/agent-memory/`).

Re-print the board path (`.ckit/state/ticket-board.html`) in that summary. By the end of a long
