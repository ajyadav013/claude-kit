...
6. **Promote the lesson** — add a durable entry to `.ckit/agent-memory/gotchas/` via `remember` if it's a reusable pitfall (e.g., "migrations must be a separate gated step in prod").
...
> Adapted from a portfolio project's incident skill; generalized to be stack-agnostic for ckit.
