...
Verify UI/UX design compliance for the invocation request.

## Steps

1. **Read the design system**: Read `ui-design-system` (foundations & index — colors, typography, spacing, icons, tokens) and its companions `ui-components` (cards, badges, buttons, form controls, states, compound components) and `ui-layout-and-motion` (page layout, motion, accessibility, page blueprints). (Installed as overlay rules when the React stack is selected.) These files are **authoritative**: if the quick-check table below ever conflicts with them, follow the rule files.

2. **Read the UX patterns docs**: Read `ux-patterns` for status expression rules, empty state guidelines, breadcrumb conventions, page blueprint selection, and data color rules, and `ux-dashboard-patterns` for chart standards, tab-bar limits, KPI-grid layouts, and the global filter strip. For responsive/native specifics, also read `mobile-design-guidelines`.
...
4. **Audit the target**: Read the component/page file(s) for `the invocation request`. Check against these rules:
...
- Design system: `ui-design-system` (foundations), `ui-components`, `ui-layout-and-motion`
- UX patterns: `ux-patterns`, `ux-dashboard-patterns`
- Mobile / responsive: `mobile-design-guidelines`
