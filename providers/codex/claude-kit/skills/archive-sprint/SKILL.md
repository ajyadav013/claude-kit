...
Archive the completed sprint for backlog item #the invocation request.

## Steps

1. **Find the planning folder**: Look in `docs/planning/` for the directory corresponding to backlog item #the invocation request.
...
2. **Verify completion**: Check that the work is actually done:
   ...
   - **Run the project's checks and confirm they actually pass** — the test suite, linter, and build, executed now (not assumed from the report). Do not archive on a red or unrun check; capture the real result. A "green" claim must cite the command + output (`quality-gates` §2.5).
