...
Generate a sprint plan for backlog item #the invocation request.
...
context-budget guidance in `agent-resilience`):
...
- **Hand off cleanly on replacement.** Before retiring an agent, capture its state to working memory
  (`.ckit/CONTINUITY.md`) and check `git status` so nothing in-flight is lost; the replacement
  resumes from that snapshot (reload, don't re-run — see `continuity`).
