...
- **Do we even need a new dependency?** That is the **§2a.5 Reuse & YAGNI Gate**
  (`mandatory-workflow`) and the `over-engineering-review` skill — walk the ladder
...
- `mandatory-workflow` (§2a.5 Reuse & YAGNI Gate) — the gate that decides *whether* a dep is needed
...
- [`library-skills`](https://library-skills.io) (MIT) — *after* you adopt, installs the library's author-shipped, version-synced skills into `.agents/skills/` so the agent uses its current API rather than stale training-data patterns (complements the Context7 live-docs MCP); for libraries that ship skills today
