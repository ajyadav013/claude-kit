...
Scope backlog item #the invocation request for implementation.

## Steps

1. **Read the backlog entry**: Read `docs/backlog/README.md` to find which horizon file contains item #the invocation request, then read that horizon file (e.g., `docs/backlog/now.md`) and find the item. Extract the title, priority, status, and full description.
...
3. **Gather context**: Based on what the backlog item describes, read the relevant reference docs:
   ...
   - For component/UI changes: read `AGENTS.md` for design system rules and component patterns
