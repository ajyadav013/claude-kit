...
> confirm-before-delete posture of `human-in-the-loop`. Not vendored.
...
Deleting files you did not author is a **confirm-tier action** (`human-in-the-loop`,
`agent-guardrails`): present the plan and get a yes before any merge-and-delete.
...
A cheaper model tier can do the read-only triage (`model-tiers`); keep the merge
...
| Metrics / before-after baselines | the benchmarks/eval doc (`evals` artifacts) |
...
4. **Promote, don't hoard.** A durable lesson goes to `agent-memory/` (`remember`), not into a doc;
   keep this aligned with the promote-don't-hoard rule in `continuity`.
...
- `human-in-the-loop` / `agent-guardrails` — deleting files you
  didn't create is confirm-tier; this skill obeys that gate
- `continuity` — promote-don't-hoard; durable lessons go to `agent-memory/`, not docs
- `model-tiers` — run the read-only triage on a cheaper tier
