...
1. **Identify what changed**:
   ...
   - If `the invocation request` is a file path: skip the scan, just refresh that specific doc.
   - If `the invocation request` contains `--since`: run `git diff --name-only HEAD~N` to find changed source files in the last N commits.
...
2. **Map changes to docs**: Determine which docs are affected by the changes. (The mapping below is
   ...
   - Component changes → the component-inventory doc, `AGENTS.md` (architecture section)
...
5. **Ask the user what to refresh**: Use pause and request user input to ask which docs to update.
...
|-----|-------------------|
| `AGENTS.md` | core source dirs, project structure |
