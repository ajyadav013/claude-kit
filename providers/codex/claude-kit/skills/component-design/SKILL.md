...
Design and implement the component: the invocation request.

## Steps

1. **Read the design system**: Read `ui-design-system` (foundations & index) and `ui-components` (component standards — cards, badges, buttons, form controls, compound components), both installed as React overlay rules — or a project's `docs/references/ui/ui-design-system.md` equivalent. Read whichever is present.
...
- Design system: `ui-design-system` (foundations) + `ui-components` + `ui-layout-and-motion` (React overlay) or `docs/references/ui/ui-design-system.md` (if present)
- UX patterns: `ux-patterns` + `ux-dashboard-patterns` (React overlay) or `docs/references/ui/ux-patterns.md` (if present)
- Rules: `frontend-best-practices`, `responsive-and-accessibility`
