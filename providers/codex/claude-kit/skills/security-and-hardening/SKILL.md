...
Companion rules: `code-organization` (auth & dependency patterns), `design-patterns` (service/repository boundaries), `documentation` (security documentation). The `security-reviewer` agent (Phase 5.4 in `mandatory-workflow`) enforces what this skill teaches.
...
> Distinct from `agent-guardrails`, which secures the *Codex agent itself*. This
...
See `references/security-checklist.md` for the full pre-commit checklist.
