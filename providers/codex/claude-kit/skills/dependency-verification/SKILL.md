...
*author-shipped, version-synced* skills into `.agents/skills/` (managed symlinks; `uvx library-skills`
...
(`agent-guardrails` §4). This section is the **canonical intake check** for any
...
| **Privilege-carrier abuse** | Frontmatter grants (`provider-native tool allowlist metadata`, a broad `tools:` list) far wider than the artifact's stated job needs. |
...
### Intake procedure (before the artifact enters `.ckit/` or `.codex/config.toml`)
...
5. **NEVER "load it to inspect it".** A stdio MCP entry **executes its command the moment a client
   loads the config** — adding an untrusted server to `.codex/config.toml` "to see what tools it has" *is*
...
- [ ] Any third-party agent-config artifact passed the intake procedure (provenance pin · structural read · hidden-content scan · credential-routing test) before entering `.ckit/` or `.codex/config.toml`, and no untrusted MCP entry was loaded "to inspect it"
