...
- Tracing setup must never crash or block service startup. Wrap initialization in the language's error guard and continue untraced on failure (`agent-resilience`).
...
- `devops-observability` — The neutral policy for observability, logging, metrics, and tracing across all stacks.
...
- `agent-resilience` — Fail-safe principles for instrumentation (never crash, degrade gracefully).
