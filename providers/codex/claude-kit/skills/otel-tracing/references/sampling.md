...
> `langfuse-llm-tracing`. The neutral policy is `devops-observability`. This is the
...
- Rules: `devops-observability` (neutral observability policy — logs, metrics, traces, correlation; backend-agnostic), `agent-resilience` (tracing must degrade gracefully and never crash the app)
