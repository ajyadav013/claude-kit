...
- **Cite evidence, never paste it** — executed output (test runs, measurements, gate proofs) lives at its `.ckit/state/` or artifact path; the spec links to it, so it stays requirements-sized for every agent that reads it.
