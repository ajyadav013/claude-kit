...
| `$deploy setup` | (Re-)run the interview and rewrite `.ckit/config/deploy.yaml` |
...
Read `.ckit/config/deploy.yaml`. If it exists, use it and **ask nothing** already answered.
...
1. **Integration branch** — the branch deployments are cut from (staging/SIT/release), if any.
   Detect: a branch named in `AGENTS.md`, or the branch recent merge commits target. Trunk-based
...
Write the answers to `.ckit/config/deploy.yaml` (create `.ckit/config/` if needed):
...
build id, findings) in `.ckit/CONTINUITY.md`, not here.
...
- **A release that carries a database migration pauses for explicit human confirmation** with a
  written rollback note before the trigger fires (see `risk-classification`).
...
non-trivial feature work, run it through the sdlc skill first and use this skill for the delivery
...
### 3. Commit
`git status -s` → if changes exist: `git diff --stat` to understand them, use `the invocation request` as
...
permission settings — Codex already gates tool execution. If the trigger command isn't
...
Checkpoint `.ckit/CONTINUITY.md` at every stage transition — config in use, iteration number,
