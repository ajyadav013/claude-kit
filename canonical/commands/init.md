...
schema_version: 1
id: init
description: Scaffold the {{kit:cli}} SDLC config ({{ref:artifact://project-instructions}} + state://root/rules, agents, skills, hooks) into this project
aliases:
- init
invocation: explicit
capabilities:
...
Install the {{kit:cli}} autonomous-SDLC configuration into the current project.
...
(`{{kit:cli}} upgrade` / `diff`). First detect whether it is on PATH (three entry points ship):

```
command -v {{kit:cli}} >/dev/null 2>&1 && echo "CKIT_CLI={{kit:cli}}" \
  || { command -v ckit >/dev/null 2>&1 && echo "CKIT_CLI=ckit" \
  || { command -v {{kit:legacy-cli}} >/dev/null 2>&1 && echo "CKIT_CLI={{kit:legacy-cli}}" \
...
`pip`/`pip3`), ask ONE {{pause_for_human:input}} with the install option first:

- **"Install {{kit:distribution}} now (Recommended)"** — on accept, run `pipx install {{kit:distribution}}`
  (fall back to `pip install {{kit:distribution}}` when pipx is absent), then **re-run the detection
...
- **"Skip"** — or if the install fails, or no installer is on PATH: STOP — do not scaffold
  ...
  re-run `{{command_alias:command://init}}`. Do not silently fall back to a partial install.
...
- Recommended: `pipx install {{kit:distribution}}`
- Or: `pip install {{kit:distribution}}`
...
2. **Otherwise — interview the user yourself, then run non-interactively.** Ask the ordered
   questions in chat ({{pause_for_human:input}} where available; every question has a default the user can
   accept):
   ...
   5. Optional MCP integrations (default: none; ids via `{{kit:cli}} list-options`)
   6. Learning capture (`off` default · `session-end-catchup` recommended · `session-end` ·
      ...
      transcripts + changed files before they choose, that `{{kit_env:no-autocapture}}=1` disables
      it at runtime, and that `{{kit:cli}} privacy-report` audits what got installed. Only record a
...
the detected CLI was `{{kit:cli}}` and the user passed `/path/to/proj --defaults`, run
`{{kit:cli}} init /path/to/proj --defaults`.
...
compatibility dispatcher only: it invokes `{{kit:cli}} init`/`ckit init` when installed and otherwise
exits non-zero after printing `pipx install {{kit:distribution}}`. Do not copy the payload with shell
`cp`/`rm` commands and do not treat `{{kit_env:basic}}` as a bypass. If the CLI cannot be installed,
STOP and report that **no project files were changed**.

After it completes:
1. Summarize what was installed — `{{ref:artifact://project-instructions}}`, `state://root/{rules, agents, skills, hooks, templates}`,
   `state://configuration`, and optional `artifact://mcp-configuration` — with counts.
2. If `{{ref:artifact://project-instructions}}` / `settings.json` / `artifact://mcp-configuration` already existed, the installer wrote a
   `{{kit:sidecar-suffix}}` sidecar instead of overwriting. Point these out and offer to merge them (or suggest
   re-running with `--force`).
3. Tell the user to **restart {{host:product}}** so the newly installed project agents, skills, and hooks load.
4. Suggest the next step: run `{{short_command:command://sdlc}} <your first task>` (or `{{command_alias:command://sdlc}}` from the plugin).
