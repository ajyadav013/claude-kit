...
schema_version: 1
id: abort
description: Abort the in-progress {{short_command:command://sdlc}} run — terminate its authoritative snapshot and clean its worktrees
aliases:
- abort
invocation: explicit
capabilities:
...
transition is `{{kit:cli}} pipeline abort`; worktree cleanup remains guided and must never remove
anything this run did not create.

1. **Confirm a run is in progress.** Run `{{kit:cli}} pipeline status` and read
   `state://continuity`. The schema-v2 snapshot is authoritative for lifecycle status; CONTINUITY
...
2. **List worktrees:** `git worktree list`. Identify **only** the worktrees this run created for its
   lanes (the `developer` lanes use the {{tool:delegate}} tool's `isolation: "worktree"`). When
   `state://pipeline-snapshot` records a `git.worktrees` map, treat it as the
...
3. **Remove them:** `git worktree remove <path>` for each identified worktree. Unchanged worktrees are
   auto-cleaned by the {{tool:delegate}} tool; this handles any that remain. Add `--force` **only** after telling
   the user exactly what uncommitted lane work would be lost and getting confirmation.
4. **Terminate the authoritative run:** run `{{kit:cli}} pipeline abort`. It atomically marks the
   ...
   `ABORTED <date> — <reason from {{request}}>` under **Current Phase** in `state://continuity`, and
   ...
   line intact so `{{short_command:command://sdlc}}` can offer RESUME instead.)
