...
5. **It's high-risk** — auth, payments, secrets, production data, migrations, or infrastructure are
   involved. Default to **asking** here even if you have a plausible default (`rule://risk-classification`).
...
> `rule://prompt-to-task-conversion`, `rule://human-in-the-loop`,
> `rule://risk-classification`. The `interview-me` and `idea-refine` skills extract true intent
