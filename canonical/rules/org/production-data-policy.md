...
4. **No exfiltration.** Don't copy production data into logs, prompts, fixtures, screenshots, or
   external services. Personal data is additionally governed by `rule://pii-policy`.

## Rules

- **Explicit, recorded approval.** Any operation on production data stops and asks a human; record who
  approved, what was approved, and when. See `rule://human-in-the-loop`.
...
> `rule://risk-classification`, `rule://pii-policy`, and
> `rule://human-in-the-loop`. Sensitive-area escalation is also wired into
