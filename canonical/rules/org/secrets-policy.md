...
> Part of {{ provider.executable.cli }}'s organization capability layer. Cross-refs `rule://agent-guardrails`
> (privilege/guardrail trips), `rule://pii-policy` (personal data handling), and
> `rule://compliance-policy` (regulatory obligations on exposure).
