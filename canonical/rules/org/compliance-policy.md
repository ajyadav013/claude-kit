...
**high** per `rule://risk-classification`). When in doubt, treat it as regulated.
...
4. **Change control** — no change reaches a protected branch without a tracked request: spec, approvals,
   evidence, and rollback notes attached. Follow `rule://branch-and-pr-policy`.
...
- **Sign-offs are scoped.** Approval covers one change in one context; re-confirm for each gated step
  (`rule://quality-gates`).
- **Secrets and PII stay protected.** Apply `rule://secrets-policy` and
  `rule://pii-policy`; a violation is an auto-Critical that blocks every gate.
...
> Part of {{ provider.executable.cli }}'s organization capability layer. Cross-refs
> `rule://risk-classification`, `rule://quality-gates`,
> `rule://secrets-policy`, `rule://pii-policy`. Enforced by the **audit-log** hook
