...
Database conventions. Installed into `artifact://rules-directory/` only when **MongoDB** is selected. Read
`rule://design-patterns` and your backend overlay rule first; this file makes the data
...
- **Expand/contract — drop fields in a later deploy.** Old and new document shapes coexist during a
  ...
  `rule://quality-gates`); the `migration-specialist` reviews every shape change against this.
