...
> `rule://postgres-patterns`. Adapted from a portfolio project's database-performance rule.
...
- Add the index in the **same migration** as the column/query (see the migration patterns in `rule://postgres-patterns`).
...
## Severity (per `rule://quality-gates`)
