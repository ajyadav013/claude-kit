...
Database conventions. Installed into `artifact://rules-directory/` only when **PostgreSQL** is selected. Read
...
- **Expand/contract — never destruct in place.** A destructive or narrowing change (`DROP COLUMN` /
  ...
  **contract** step: it ships in a **later** release than the code that stops using the old shape
  ...
  table-locking migration on a large table is at least **High** (per `rule://quality-gates`);
