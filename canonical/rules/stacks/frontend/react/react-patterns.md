...
Stack-specific conventions for the frontend. This overlay is installed into `artifact://rules-directory/` only
when the **React** frontend is selected. Read the generic
`rule://frontend-best-practices`, `rule://code-organization`, and
`rule://responsive-and-accessibility` first; this file makes them concrete for React.
...
Run the project's own scripts for these tasks (see the **Commands** section of `artifact://project-instructions`):
...
- **Type everything.** No `any`; exported functions and hooks have explicit return types and doc
  comments per `rule://documentation`. Type-check and lint must pass.
...
technique in `rule://responsive-and-accessibility` and the `accessibility-review` skill. Two
...
Route by what changed (run the project's test command scoped to the area; see `artifact://project-instructions` Commands):
