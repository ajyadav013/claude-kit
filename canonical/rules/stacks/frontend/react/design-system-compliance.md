...
Stack-specific design-system enforcement for the frontend. Installed into `artifact://rules-directory/` only when
the **React** frontend is selected; it complements `react-patterns.md`,
`rule://responsive-and-accessibility`, and the `ui-ux-design` skill (the agnostic
...
> file in `artifact://rules-directory/`. Those are the source of truth; this file is the short, always-on
...
and theme-scope leaks. Severity per `rule://quality-gates`; the broader UX/visual review
