...
This is **distinct** from `rule://agent-resilience`, which is about the *coding agent's own
...
SLO (`rule://devops-observability`); or any decision that depends on ordering events across
more than one machine. **Skip (note why in `state://continuity`)** for a pure local computation, a
...
A chaos finding is a real defect: classify it by `rule://quality-gates` severity and route the
...
- **`rule://agent-resilience`** — the same patterns applied to the *coding agent's* own run
  (the agent surviving a tool/network failure), not the product's services. Read both; don't conflate.
- **`rule://devops-observability`** — resilience is unmeasurable without SLOs, burn-rate
  alerting, and the failure-domain progressive-rollout discipline; chaos experiments assert against the
  SLOs defined there.
- **`rule://quality-gates`** — a chaos finding or an unhandled cascading-failure path is a
  finding classified and gated like any other.
- **`rule://testing`** — deterministic simulation testing injects faults *in a reproducible
