...
`docs/org-capabilities.md` of the {{ provider.executable.cli }} repo), which are explicit and auditable — not a quiet
config override.

## Rules

- **Never exceed the active level.** If a task needs more autonomy than granted, stop and ask — do not
  silently escalate. See `rule://human-in-the-loop`.
- **Risk can lower the effective ceiling.** High-risk or restricted work (auth, payments, secrets,
  ...
  of level. See `rule://risk-classification`.
...
> Part of {{ provider.executable.cli }}'s organization capability layer. Cross-refs `rule://human-in-the-loop`,
