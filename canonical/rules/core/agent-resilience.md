...
- `rule://quality-gates` owns *gate* retry **budgets** (a review/test failing on its merits
...
- `rule://resilience-engineering` applies the same vocabulary (circuit-breaker, fallback,
...
| **Fallback** | A primary tool/source/path is unavailable | Have a defined alternative (another source, a simpler method, manual steps) and say you used it. A planned model tier that fails its pre-fan-out probe falls back one tier, recorded (`rule://model-tiers` → Probe before fan-out). |
...
| **Checkpointing** | Long runs, before risky steps, pre-compaction | Write `state://continuity` so a crash/compaction resumes from the last good state, not from zero. See `rule://continuity`. |
...
- **At the 3rd failed attempt — STOP.** Do not attempt a fourth blind fix. Switch modes: re-derive
  the root cause from first principles (`skill://debugging-and-error-recovery`), and
  ask the harder question — is the *approach or architecture* wrong, not the line?
- **Then escalate** (`rule://human-in-the-loop`) with what you tried, what each attempt
...
`rule://quality-gates` bounds how many times a finding may cycle review→fix across the
...
1. **Fail loud, never silent.** A swallowed error that lets work continue on bad state is worse than a
   ...
   hide a failure is an auto-Critical in `rule://quality-gates`.)
2. **Bounded everything.** Every retry/recovery loop has a limit. Exhausting it routes to a human
   (`rule://human-in-the-loop`), it does not spin.
...
4. **Promote recurring failures.** A failure mode worth avoiding next time goes to `state://agent-memory/` via
   `remember`.
...
- **`rule://continuity`** — the checkpoint that makes resume-after-failure possible.
- **`rule://agent-guardrails`** — malformed/hostile input is both a guardrail and a
  resilience event; handle the validation there, the recovery here.
- **`rule://human-in-the-loop`** — the escalation target when recovery budgets are exhausted.
