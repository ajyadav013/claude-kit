...
`rule://rarv-cycle` says **reflect and verify before handoff**; this rule says **how to
...
- **Version it and test it.** Store the prompt as a file under version control so changes are
  reviewable and diffable, and write **evals** against it (`rule://evals`) so a "small wording
...
4. **Self-consistency is the single-agent form of blind review.** When you must decide alone, generate
   ...
   (`rule://quality-gates`) or the `doubt-driven-development` skill.
5. **Stop and ask instead of guessing.** If reasoning bottoms out in a genuine ambiguity, that's a
   human decision point — see `rule://human-in-the-loop`. Reasoning harder cannot
...
Reasoning has a cost; allocate it deliberately (see also `rule://agent-resilience` for the
...
- **Model tier is a per-agent choice.** When an agent's role is a narrow specialist scan, choose the
  semantic fast model tier in its agent definition; reserve the deepest tier for architecture,
  security, and decisive reviews. Provider adapters own the native field mapping — see
  `rule://model-tiers` for the concrete per-agent tier policy.
- **Prune context before you reason.** Summarize or drop what the current decision doesn't need —
  isolate large reads behind a read-only research subagent so the main context stays clear enough
...
- **`rule://rarv-cycle`** — RARV is the *discipline* (reflect + verify before handoff); this
  rule supplies the *reasoning methods* RARV's Reason and Act steps draw on.
- **`rule://quality-gates`** — blind review + Devil's Advocate are the multi-agent escalation
  of self-consistency for high-stakes verdicts.
- **`rule://agent-guardrails`** — reason over inputs, but never *trust* them; fetched/tool
