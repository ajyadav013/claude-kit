...
Feature-scale work stays on the normal pipeline in `rule://mandatory-workflow`. Everything
below *adds to*, and never replaces, the gates in `rule://quality-gates`, the risk tiers in
`rule://risk-classification`, and the stops in `rule://human-in-the-loop`.
...
Use the `risk-classifier` agent / `rule://risk-classification` tiers to place each
...
Irreversible steps follow `rule://human-in-the-loop`, with this sharpening: the worker
...
`rule://agent-resilience`; this section is about *scope* surprises, not tool errors.)
...
world: `artifact://project-instructions`, affected `artifact://rules-directory/` prose, skills, runbooks, and durable lessons into
...
Match each worker's model to its task per `rule://model-tiers`: audits and mechanical
...
`rule://goal-setting-and-monitoring` §4. That makes both *scale and spend* vetoable
...
- **`rule://mandatory-workflow`** — the feature pipeline each worker still follows *inside*
  its unit of work; this rule sequences units into waves above it.
- **`rule://quality-gates`** — the severity model every wave gate applies.
- **`rule://risk-classification`** — the tiers that order the waves.
- **`rule://human-in-the-loop`** — the stop points; §5 here is its inventory sharpening.
- **`rule://agent-resilience`** — worker crash/retry handling.
- **`rule://model-tiers`** — per-worker model selection.
- **`rule://continuity`** — the manifest complements, never replaces, state://continuity and
