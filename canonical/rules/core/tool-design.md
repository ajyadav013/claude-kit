...
when the tool is actually used (the model behind `artifact://skills-directory/`). This keeps the attention budget on
the task — see `skill://context-engineering`.
...
- **Make every compression reversible.** When a tool truncates, samples, or dedups, it must write the
  ...
  **retrieval path** — never silently drop items. A compression the agent can't undo is a deletion.
  (The consumer-side twin of this rule lives in `skill://context-engineering`.)
...
(`artifact://skills-directory/_references/orchestration-patterns.md`).

## 6. Safe and idempotent

A tool is a **privilege boundary** (`rule://agent-guardrails`): grant least privilege,
validate inputs, make repeated calls safe to retry (idempotent), and gate destructive or
outward-facing actions behind `rule://human-in-the-loop`.
...
- **More agents is not more throughput (Brooks's law for agents).** Coordination overhead grows with
  ...
  next time. (The parallel-lane model lives in `rule://mandatory-workflow`.)
...
- **Plan before execute.** For a multi-step goal, let the agent sequence the calls it intends *first*,
  ...
  steps are independent (next bullet). (Mirrors `rule://reasoning-techniques`.)
...
- **Persist workflow state across turns/steps.** A flow that spans turns or agents needs durable state
  ...
  `rule://continuity` plays for the session.
...
4. **Eval your tools too** — a tool's effect on agent success is measurable; see `rule://evals`.
...
- **`rule://agent-guardrails`** — tools as privilege boundary; least privilege; gating.
- **`rule://mandatory-workflow`** — the parallel-lane model that §7's Brooks's-law caution
  applies to, and the durable cross-turn state §8 calls for.
- **`rule://continuity`** — durable workflow state across turns/handoffs (§8).
- **`rule://reasoning-techniques`** / **`model-tiers.md`** — tool use is part of how an agent reasons.
- **`rule://evals`** — measure whether a tool change actually helps.
