...
**Skip (note in state://continuity why)** for pure-internal changes with no deployment or observability surface — a refactor behind an unchanged interface, a copy tweak, a test-only change. Fast-track (Mode D) skips both unless infra/observability is the actual subject of the fix.
...
- [ ] CI config is valid and includes lint + type-check + unit tests for the changed stack(s) (CI pipeline config files — editing those requires user approval per artifact://project-instructions).
...
Findings classified by `rule://quality-gates` severity; Critical/High/Medium block.
...
- [ ] **Load verified against the SLO** — for a change to a hot / SLO-bearing backend path, an empirical load run (drive `skill://load-testing`) was executed against the defined SLO and **met** its p95/p99 latency + error-rate + throughput budgets; record it under `docs/performance/` and link it from the feature SLO doc. *Skip (note why in `state://continuity`) for changes with no concurrency-sensitive surface.* A budget breach is **High** (`rule://quality-gates`).
...
- **Gate on the delta, with a tolerance band.** Fail the check when the treatment is worse than control
  ...
  isn't flaky. A breach is **High** (`rule://quality-gates`) — same severity as a Load-vs-SLO
...
- **Scope it.** Run it for changes to a concurrency-sensitive or SLO-bearing path; *skip (note why in
  `state://continuity`)* for changes with no performance surface. It complements, not replaces, the one-shot
...
- **Pause on the alarm automatically.** Wire the rollout to the SLO signals above (burn-rate alert,
  ...
  a breach is **High** (`rule://quality-gates`) and routes the fix back to the dev lane.
- **Scope it.** Worth the machinery for a multi-domain / SLO-bearing service; *skip (note why in
  `state://continuity`)* for a single-instance or non-critical surface.
...
- Both agents follow the **RARV cycle** (`rule://rarv-cycle`) and update `state://continuity` at handoff.
...
- Editing CI pipeline config, package manifests, or other project-wide files still requires explicit user approval (artifact://project-instructions §"Files that require user approval").
