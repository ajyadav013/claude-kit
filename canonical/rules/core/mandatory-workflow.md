...
`rule://linting-and-formatting` and `rule://testing`). The canonical
...
| Migration / repo-wide refactor / many subsystems / any irreversible step (production data, schema, deletion sweep) | **Wave Orchestration** — `rule://wave-orchestration` (each unit inside a wave still runs one of the two workflows above) |
...
> contract; see `rule://wave-orchestration` → "Optional host-native workflows as the wave
> substrate".
...
an agent in `artifact://agents-directory/`, not a hat the main session puts on.** Running the pipeline means
spawning them with the {{ provider.tool.delegate }} capability.
...
- **state://continuity** — read `state://continuity` at the start of every turn and write it
  ...
  to `state://agent-memory` via the `remember` skill. See `rule://continuity`.
- **RARV** — every step runs Reason → Act → Reflect → Verify; never hand off before Verify is
  green. See `rule://rarv-cycle`.
- **Severity** — classify findings Critical/High/Medium/Low/Cosmetic; a gate passes only with
  zero Critical/High/Medium open. See `rule://quality-gates`.
- **Agent operating disciplines** — every agent also follows, throughout both workflows: how to
  reason (`rule://reasoning-techniques`); measurable goals, monitoring, and prioritization
  (`rule://goal-setting-and-monitoring`); safe handling of untrusted inputs/outputs and
  least-privilege tools (`rule://agent-guardrails`); recovery from tool/agent failures
  (`rule://agent-resilience`); and when to stop for a human
...
handled by a dedicated agent in `artifact://agents-directory/`.
...
stable finding schema from `rule://quality-gates` §3. Preferences, stylistic alternatives, and
...
applies the decision-rights table in `rule://quality-gates`, and sends the writer **one** consolidated
...
**Output:** a story breakdown (written where the spec lives, or to `state://workflow/`) with stable
...
to it — see `skill://task-tracker-sync`.
...
fan-out (`skill://sdlc` → Story-group fan-out). **Batching:** up to **3** stories
...
(`rule://risk-classification` → Story-level routing) and its own ticket (1g).
...
Using `skill://ticketing-and-traceability`:
...
- **Work-log every step.** As each story is implemented and validated, append an entry to its ticket:
  ...
  expensive to reverse graduates to an ADR (`skill://documentation-and-adrs`).
...
at each stage transition make the resume cheap (`rule://continuity`). Calibrate to story
...
the user's others; `git worktree remove`, see `skill://git-workflow-and-versioning`).
Before writing code, read the approved spec + dev docs,
the relevant `artifact://rules-directory/*` for your stack, and EVERY file you plan to modify — in full.
...
`skill://simplification-debt`) — so it is traceable, not silent debt.
...
- **Accessibility (UI):** keyboard access, alt text, ARIA labels, focus management, color is
  never the only indicator, sufficient contrast. See `rule://responsive-and-accessibility`.
...
suite once per persona or lane (`rule://quality-gates` §2.5).
...
merits-and-costs balance sheet; any accepted cost is recorded in `state://continuity` with an owner and
a revisit trigger. See `rule://quality-gates`.
...
noted reason otherwise — see `rule://devops-observability`):
...
1. **Final quality gate:** require the project's lint → type-check → unit tests → E2E tests → build
   ...
   reuse key (`rule://quality-gates` §2.5), cite it; otherwise run it now. Never rerun solely because
...
`rule://human-in-the-loop` for the full set of decision points and the escalation protocol.
...
barrels/index files, `artifact://project-instructions`, and `artifact://rules-directory/*`. Define the exact list for your project.
