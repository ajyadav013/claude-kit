...
(`rule://human-in-the-loop`, exhausted budgets).
...
Where the agent is installed, a gate reached by unanimous PASS is not PASS until the Devil's Advocate returns CONFIRMED or CONFIRMED-WITH-COSTS. See `agent://devils-advocate` (present in the standard and enterprise profiles).
