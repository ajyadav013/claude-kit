...
See `rule://responsive-and-accessibility` for UI-specific accessibility requirements.
...
`skill://test-driven-development`) only exercise inputs you *thought of*. **Fuzzing** generates
...
hardening in `skill://security-and-hardening`.
...
- **Make it continuous, as a CI gate.** Run a short, code-change-scoped fuzz pass on each PR (catches
  ...
  `skill://ci-cd-and-automation` skill alongside lint/test.)
...
cold start** brings everything up healthy (`rule://devops-observability`). Suite-scoped
...
- **Scope it.** DST earns its architectural cost for **stateful concurrent/distributed** components
  (consensus, replication, schedulers, queues, transaction logic). *Skip (note why in `state://continuity`)*
...
fault injection of chaos engineering (`rule://resilience-engineering`).
...
- Integration/E2E tests run in the testing phase of the SDLC pipeline (see `rule://mandatory-workflow`)
