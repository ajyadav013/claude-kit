...
- **Validate before use.** Check that an input is the shape/type/range you expected before acting on
  it. Malformed or surprising input is a signal to slow down (see `rule://agent-resilience`),
  not to improvise.
...
- **No secret leakage.** Never emit credentials, tokens, keys, or `.env` contents into reports, logs,
  ...
  security gate — `rule://quality-gates`; this clause is about not leaking via agent output.)
- **Truthful status.** Never report a check as passing without running it; never claim green when it
  ...
  as fabricated (`rule://quality-gates` §2.5).
...
- **Block — never autonomously.** Force-push, rewriting already-published history, deleting a branch
  ...
  `rule://risk-classification`. Several are also stopped deterministically by
...
- **Confirm — pause and get a yes.** Deleting or overwriting a file you did not create, applying a
  ...
  wait — see `rule://human-in-the-loop`.
...
- **Keep a verifiable audit trail.** Record what was done, on whose behalf, and under what
  ...
  `rule://human-in-the-loop`) especially must leave a trail.
- **Authorization policy is data, updatable without a redeploy.** What an agent may do should be a
  ...
  not a constant baked into the agent. The active `rule://autonomy-levels` tier and
  `rule://risk-classification` are such runtime controls.
...
3. **Guardrails evolve.** New manipulation patterns get promoted to `state://agent-memory/` via `remember` so
...
- **`rule://human-in-the-loop`** — where a tripped guardrail escalates to a human.
- **`rule://agent-resilience`** — malformed/hostile input often coincides with failures;
  the two rules are applied together.
- **`rule://quality-gates`** — product-security severity & the secret = auto-Critical rule.
- **`rule://autonomy-levels`** / **`risk-classification.md`** — the runtime controls that
