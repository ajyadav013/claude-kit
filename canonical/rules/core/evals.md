...
This is distinct from `rule://testing` — that rule covers deterministic product tests
...
strategies. (Mirror of the RARV "verify the result" stance in `rule://rarv-cycle`.)
...
- **Panel of judges for multi-dimensional or high-stakes scoring.** Instead of one judge rating
  ...
  of multi-agent blind review in `rule://quality-gates`).
...
- **Separate metrics that *measure* from metrics that *gate*.** A **measurement** metric records a
  ...
  because the correctness gate executed it. Wire only gate metrics into `rule://quality-gates`;
...
>
> To measure the **{{ provider.executable.cli }} pipeline itself** (the same tasks run with vs without the gates, and
> which gate caught each defect), the {{ provider.executable.cli }} repo ships a fill-in template — `docs/eval-harness.md`
...
A single-turn eval will not catch this; {{ provider.executable.cli }}'s own agents run over long multi-turn sessions
...
- **A/B against the skill's absence.** Each case runs *with* and *without* the skill and is judged
  ...
  with/without method as the kit-level harness (`docs/eval-harness.md` in the {{ provider.executable.cli }} repo),
  applied per skill.
...
2. **Evals are how you adopt a new model.** Re-run the suite before re-tiering an agent
   (`rule://model-tiers`); a cheaper model that holds the eval is a free win.
3. **Evals are living infrastructure with an owner** — versioned, in the repo, run in CI where possible.

## Relationship to other rules

- **`rule://testing`** — deterministic product tests; evals are its probabilistic sibling.
- **`rule://goal-setting-and-monitoring`** — eval pass-rates are measurable success criteria.
- **`rule://quality-gates`** — a regression-eval drop is a gate-failing signal.
- **`rule://model-tiers`** — re-run evals before changing an agent's model.
