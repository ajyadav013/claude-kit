...
The feature spec already *defines* acceptance criteria (`spec-doc-writer`, `rule://mandatory-workflow`
...
Record them in `state://continuity` (and for features, they live in the spec). If you cannot make a
...
- **Track, don't assume.** As stages complete, check actual results against the criteria — the RARV
  **Verify** step (`rule://rarv-cycle`) is where each criterion gets proven.
- **Watch the process signals** in `rule://quality-gates` (gate first-pass rate, fix
...
- **Hang the spans off events you already have.** The host's hooks (prompt-submit, pre/post tool use,
  ...
  instrumenting business logic. {{ provider.executable.cli }}'s own `hooks/` and its orchestrator/sub-agent structure map
  directly onto this tree.
- **Record the attributes that make a run debuggable:** which tool/model, token cost, latency, and
  outcome per node — the same signals §2 monitors and `rule://quality-gates` trends. Capture
...
- **Use a stable attribute vocabulary** (a documented convention — e.g. the emerging GenAI semantic
  conventions) so runs stay comparable across sessions and tools, the way `rule://evals`
...
(`rule://evals`) and trend. (This is *agent-run* telemetry; **app and LLM** observability
...
workflows, and eval harnesses (`rule://evals`) — not in live incident response, where
`rule://agent-resilience` and the `incident-responder` path apply instead.
...
budget: stop spawning, checkpoint state to `state://continuity`, and escalate per
`rule://human-in-the-loop` — never silently run past the number you announced. (Wave-scale
declarations live in `rule://wave-orchestration`'s manifest.)
...
1. **No work without a checkable goal.** Even a fast-track fix states what "fixed" means and how it's
   proven.
2. **Criteria live in working memory.** Keep them in `state://continuity` so they survive
...
- **`rule://rarv-cycle`** — Verify proves each criterion; this rule defines the criteria.
- **`rule://continuity`** — where criteria + progress are recorded and monitored.
- **`rule://quality-gates`** — process signals = the monitoring instrumentation.
- **`rule://human-in-the-loop`** — unmeasurable/ambiguous criteria and major re-scoping
  escalate here.
