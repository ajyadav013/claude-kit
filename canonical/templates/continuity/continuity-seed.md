# CONTINUITY — Working Memory

> Seed template. The `load-continuity.sh` session-start hook copies this to `state://continuity`
> (gitignored) on first run. Overwrite the sections below as work progresses — keep it short and truthful.
> Budget: stay under ~8,000 bytes (~150 lines) — what the hook injects **uncut**. When a phase
> completes, compress it here and move the detail to `state://workflow/continuity-archive.md`
> (append-only cold storage, never auto-loaded).
> Protocol: read at turn start, write at turn end. See `rule://continuity`.

## Current Phase
[none — idle]

## Active Tasks
- [none]

## Completed (this session)
- [none]

## Decisions Made
- [none]

## Mistakes & Learnings
- [none]  (promote durable ones to agent-memory via the remember skill)

## Attempted & Ruled Out (this session)
- [none]  (approach — why ruled out; do not re-propose without new evidence)

## Next Steps
1. [await next requirement]

## Open Questions
- [none]

## Blocked Items
- [none]

## Modified Files
- [none]

## Repo State (from commands, never from memory)
- branch: [unknown — run git to derive]   HEAD: [unknown]   dirty: [unknown]
- PR: [none]

## Test/Build Status
- backend: [unknown]   frontend: [unknown]
