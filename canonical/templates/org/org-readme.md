...
This directory is the **org capability layer** {{ provider.executable.cli }} installs when you scaffold with
...
> (`{{ provider.path.skills }}`, `{{ provider.path.agents }}`, `{{ provider.path.rules }}`) so {{ provider.name.host }} picks them up normally. This
...
|-------|----------|---------|
| **Project** | `{{ provider.path.config_root }}`, `{{ provider.path.project_instructions }}`, `{{ provider.path.mcp_config }}`, this README — committed | what this repo needs; the source of truth per repo |
| **User** | `{{ provider.path.user_config }}` (per developer, not committed) | personal preferences, personal skills, local overrides |
...
see `rule://autonomy-levels` "Precedence"; the named exceptions are the bypass contract in
...
Every pack operates under the project's **autonomy level** (`rule://autonomy-levels`) and
**risk classification** (`rule://risk-classification`). High-risk or restricted work (auth,
...
- **Version & roll out:** packs are versioned; record changes in a changelog and roll out repo-by-repo
  with `{{ provider.executable.cli }} diff` / `{{ provider.executable.cli }} upgrade` (your edits are backed up, never silently overwritten).
...
- **Measure adoption & quality:** see the "Metrics" section in `{{ provider.path.sdlc_readme }}`.
