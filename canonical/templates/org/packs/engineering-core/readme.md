...
Use `{{skill_invocation:skill://code-simplification}}` to simplify the billing service without changing behavior.
Use `{{skill_invocation:skill://test-driven-development}}` to add regression coverage for failed password-reset links.
```
