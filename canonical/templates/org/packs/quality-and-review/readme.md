...
This pack bundles components that already ship with {{ provider.executable.cli }} (reused, not duplicated). It introduces
no competing agents — every reviewer and tester role maps to an existing pipeline agent.

| Need | Use |
|------|-----|
| Review a PR / change for correctness and quality | `command://code-review-and-quality` → `sdlc-code-reviewer` |
| Plan and write tests before/with the change | `command://test-driven-development` → `tester` |
| Add or strengthen unit coverage | `command://unit-test` → `unit-tester` |
...
| Security review of the change | `command://security-verification` (`threat-model` for design risk) |
| Performance review / regression check | `command://performance-optimization` |
| Accessibility review | `command://accessibility-review` |
| Stress-test a unanimous PASS (anti-sycophancy) | `devils-advocate` |
| Probe assumptions before approving | `command://doubt-driven-development` |
| Manual / smoke check before sign-off | `command://manual-test`, `command://smoke-test` |
...
## Examples
```
Use `{{skill_invocation:skill://code-review-and-quality}}` to check the latest change to the items service.
Use `{{skill_invocation:skill://test-driven-development}}` to add regression coverage for the failed-login path.
Use `{{skill_invocation:skill://security-verification}}` to review the new file-upload handler.
Use `{{skill_invocation:skill://accessibility-review}}` to audit the new settings screen.
```
...
human approval before the gate counts (`rule://risk-classification`).
