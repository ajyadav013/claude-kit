...
This pack reuses the security components that already ship with {{ provider.executable.cli }} (no competing agents) and
adds four policy rules plus three hooks at the org layer.

| Need | Use |
|------|-----|
| Harden code / fix an insecure pattern | `command://security-and-hardening` → `security-reviewer` |
| Verify a change is safe before merge | `command://security-verification` → `security-reviewer` |
| Model threats for a feature | `command://threat-model` → `security-reviewer` |
| Audit dependencies for known risk | `command://security-verification` → `dependency-scanner` |
...
## Examples
```
Use `{{skill_invocation:skill://security-and-hardening}}` to harden the session-handling path before launch.
Use `{{skill_invocation:skill://threat-model}}` to map abuse cases for the new public upload endpoint.
Use `{{skill_invocation:skill://security-verification}}` to flag risky or outdated packages in the data-store layer.
```
