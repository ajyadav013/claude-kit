...
This pack bundles components that already ship with {{ provider.executable.cli }} (reused, not duplicated) plus a few added
...
| Convert a raw prompt into a safe, scoped task | `command://prompt-to-safe-task` (see `prompt-to-task-conversion.md`) |
