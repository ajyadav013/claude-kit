...
> Installed by **{{ provider.executable.cli }}**. This file is the entry point and is loaded into context every
> session, so it is kept lean — the full pipeline, gating rules, agent roles, and rule details
> live on-demand in `{{ provider.path.rules }}`, `{{ provider.path.agents }}`, and `{{ provider.path.skills }}` (cited inline below).
...
- Comments explain *why*, never narrate *what just changed* ("// added this") — see
  `rule://documentation` §6. Reference code as `path:line` in notes and handoffs.
...
`rule://mandatory-workflow`, and the severity/review/gate model lives in
`rule://quality-gates`. Read those before driving a phase.

**Profile gating:** the phases below run against the agents your chosen profile actually installed
(`lean ⊂ standard ⊂ enterprise` — check `{{ provider.path.agents }}`). A phase whose reviewer agents aren't
...
- **Working memory:** read/write `state://continuity` every turn and at each stage transition so
  work survives context compaction and new sessions. Distinct from `state://agent-memory/` (durable
  learnings). See `rule://continuity`.
...
they keep their full text at `{{ provider.path.rules }}<name>.md`, so **you can open any of them by name at
...
`command://sdlc` when you want the full pipeline. The failure this prevents is not idleness but a
...
> yours. Add matching rule files under `{{ provider.path.rules }}` and reference them from the relevant
> agents.
