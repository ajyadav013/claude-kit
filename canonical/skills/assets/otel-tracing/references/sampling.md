...
> `langfuse-llm-tracing`. The neutral policy is `rule://devops-observability`. This is the
...
- Rules: `rule://devops-observability` (neutral observability policy — logs, metrics, traces, correlation; backend-agnostic), `rule://agent-resilience` (tracing must degrade gracefully and never crash the app)
