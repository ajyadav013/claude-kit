...
> [`athola/{{external_literal:external-skill-collection}}`](https://github.com/athola/{{external_literal:external-skill-collection}}) `doc-consolidation`
> skill (© 2025 athola). Re-cast around this kit's `docs/decisions/` ADR layout, CHANGELOG, and the
> confirm-before-delete posture of `rule://human-in-the-loop`. Not vendored.
...
Deleting files you did not author is a **confirm-tier action** (`rule://human-in-the-loop`,
`rule://agent-guardrails`): present the plan and get a yes before any merge-and-delete.
...
A cheaper model tier can do the read-only triage (`rule://model-tiers`); keep the merge
...
| Metrics / before-after baselines | the benchmarks/eval doc (`rule://evals` artifacts) |
...
4. **Promote, don't hoard.** A durable lesson goes to `agent-memory/` (`remember`), not into a doc;
   keep this aligned with the promote-don't-hoard rule in `rule://continuity`.
...
- `rule://human-in-the-loop` / `rule://agent-guardrails` — deleting files you
  didn't create is confirm-tier; this skill obeys that gate
- `rule://continuity` — promote-don't-hoard; durable lessons go to `agent-memory/`, not docs
- `rule://model-tiers` — run the read-only triage on a cheaper tier
