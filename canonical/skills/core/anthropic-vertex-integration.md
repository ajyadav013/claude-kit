---
schema_version: 1
id: anthropic-vertex-integration
description: '{{external_assistant:title}} on Vertex AI via the AnthropicVertex SDK — lazy ADC client init, env-based project/region resolution, async helpers, backoff retry. Use when calling {{external_assistant:title}} via GCP Vertex AI (not direct Anthropic API).'
invocation: implicit
capabilities: []
request_input:
  mode: none
...
Integrate {{external_assistant:title}} on Vertex AI using the AnthropicVertex SDK with lazy authentication, resilient helpers, and optional tracing.

## When to use

- Calling {{external_assistant:title}} via Google Cloud Vertex AI (not the direct Anthropic API with API keys)
...
- Adding Langfuse observability to {{external_assistant:title}} completions (latency, input/output, errors)
...
3. **Region/location resolution**: `_resolve_location(location)` checks `location` parameter → `VERTEX_LOCATION` or `GOOGLE_CLOUD_LOCATION` env vars → a default (e.g., `us-central1`, `us-east5`). {{external_assistant:title}} on Vertex AI is available in specific regions; check Vertex AI docs for current list.
...
11. **Model selection via env vars**: define a settings singleton with `{{external_literal:external-model-env-balanced}}` (default `{{external_literal:external-model-current-balanced}}`), `{{external_literal:external-model-env-deep}}`, etc. Use `model = model or settings.sonnet_model` in helpers. Current Vertex AI {{external_assistant:title}} models: `{{external_literal:external-model-current-balanced}}`, `{{external_literal:external-model-versioned-balanced}}`, `{{external_literal:external-model-legacy-balanced}}`, `{{external_literal:external-model-legacy-fast}}`.
...
```python
# llm/vertex_{{external_assistant:lower}}.py
import asyncio
import os
...
DEFAULT_VERTEX_MODEL = "{{external_literal:external-model-current-balanced}}"
...
class _CompletionTracer:
    """One-shot tracer for a {{external_assistant:title}} completion."""
...
def trace_completion(
    *,
    ...
    """Context manager for tracing one {{external_assistant:title}} completion to Langfuse.

    Usage:
        with trace_completion(name="extract", model="{{external_literal:external-model-current-balanced}}") as tracer:
