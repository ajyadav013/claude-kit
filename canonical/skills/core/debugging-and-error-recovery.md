...
**Safety — this is a `block`/`confirm`-tier action (`rule://agent-guardrails`).** Attaching
...
(`rule://human-in-the-loop`). Keep overhead low (sample, time-box) and **always detach** — a
