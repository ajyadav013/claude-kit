...
request_input:
  mode: optional
  hint: '[component or page name]'
pause_for_human: []
references:
- rule://mobile-design-guidelines
- rule://ui-components
- rule://ui-design-system
- rule://ui-layout-and-motion
- rule://ux-dashboard-patterns
- rule://ux-patterns
---

Verify UI/UX design compliance for {{request}}.

## Steps

1. **Read the design system**: Read `rule://ui-design-system` (foundations & index — colors, typography, spacing, icons, tokens) and its companions `rule://ui-components` (cards, badges, buttons, form controls, states, compound components) and `rule://ui-layout-and-motion` (page layout, motion, accessibility, page blueprints). (Installed as overlay rules when the React stack is selected.) These files are **authoritative**: if the quick-check table below ever conflicts with them, follow the rule files.

2. **Read the UX patterns docs**: Read `rule://ux-patterns` for status expression rules, empty state guidelines, breadcrumb conventions, page blueprint selection, and data color rules, and `rule://ux-dashboard-patterns` for chart standards, tab-bar limits, KPI-grid layouts, and the global filter strip. For responsive/native specifics, also read `rule://mobile-design-guidelines`.
...
4. **Audit the target**: Read the component/page file(s) for `{{request}}`. Check against these rules:
...
- Design system: `rule://ui-design-system` (foundations), `rule://ui-components`, `rule://ui-layout-and-motion`
- UX patterns: `rule://ux-patterns`, `rule://ux-dashboard-patterns`
- Mobile / responsive: `rule://mobile-design-guidelines`
