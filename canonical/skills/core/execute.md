...
schema_version: 1
id: execute
description: Takes raw text/requirement, optimizes it into a precise prompt, then executes it immediately. Use when you have a rough idea and want {{host:title}} to refine and act on it in one step.
invocation: implicit
capabilities: []
request_input:
  mode: required
  hint: <your raw requirement text>
pause_for_human: []
references:
- artifact://project-instructions
---

You received raw text from the user: {{request}}
...
Now execute the optimized prompt as if the user had typed it directly. Follow all project rules ({{ref:artifact://project-instructions}}, mandatory workflow, etc.) and complete the task end-to-end.
