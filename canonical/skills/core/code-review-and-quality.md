...
- **Review each bundle in isolated context, concurrently.** Give each bundle its own focused context
  ...
  `rule://mandatory-workflow` lane model, within the Brooks's-law caution in
  `rule://tool-design` §7). Context gathered for one bundle is for *understanding only* — a
...
`rule://quality-gates` (§2.5): an uncited finding is a TODO, not a finding.
...
- For detailed security review guidance, see `artifact://skill-reference-security-checklist`
- For performance review checks, see `artifact://skill-reference-performance-checklist`
