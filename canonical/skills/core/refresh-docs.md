...
1. **Identify what changed**:
   ...
   - If `{{request}}` is a file path: skip the scan, just refresh that specific doc.
   - If `{{request}}` contains `--since`: run `git diff --name-only HEAD~N` to find changed source files in the last N commits.
...
2. **Map changes to docs**: Determine which docs are affected by the changes. (The mapping below is
   ...
   - Component changes → the component-inventory doc, `{{ref:artifact://project-instructions}}` (architecture section)
...
5. **Ask the user what to refresh**: Use {{pause_for_human:input}} to ask which docs to update.
...
|-----|-------------------|
| `{{ref:artifact://project-instructions}}` | core source dirs, project structure |
