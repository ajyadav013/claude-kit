...
```
src/components/
  {{domain_literal:task-list-component}}/
    {{domain_literal:task-list-component}}.tsx          # Component implementation
    {{domain_literal:task-list-component}}.test.tsx     # Tests
    {{domain_literal:task-list-component}}.stories.tsx  # Storybook stories (if using)
...
<Card>
  ...
  <CardBody>
    <{{domain_literal:task-list-component}} tasks={tasks} />
  </CardBody>
</Card>
...
<Card
  ...
  headerVariant="large"
  bodyPadding="md"
  content={<{{domain_literal:task-list-component}} tasks={tasks} />}
/>
```
...
export function {{domain_literal:task-list-component}}Container() {
  const { tasks, isLoading, error } = useTasks();

  if (isLoading) return <{{domain_literal:task-list-component}}Skeleton />;
  if (error) return <ErrorState message="Failed to load tasks" retry={refetch} />;
  if (tasks.length === 0) return <EmptyState message="No tasks yet" />;

  return <{{domain_literal:task-list-component}} tasks={tasks} />;
}

// Presentation: handles rendering
export function {{domain_literal:task-list-component}}({ tasks }: { tasks: Task[] }) {
  return (
...
function {{domain_literal:task-list-component}}({ tasks }: { tasks: Task[] }) {
  if (tasks.length === 0) {
    return (
...
function {{domain_literal:task-list-component}}Skeleton() {
  return (
...
For detailed accessibility requirements and testing tools, see `artifact://skill-reference-accessibility-checklist`.
