...
schema_version: 1
id: threat-model
description: Structured threat model for security-relevant features (auth, payments, file upload, multi-tenant, LLM/AI). Enumerate assets, STRIDE threats, mitigations. Do NOT use to write code (use security-and-hardening).
invocation: implicit
capabilities:
- delegation
request_input:
...
**Risk tier:** high (security work — see `rule://risk-classification`).
...
6. **If the feature calls a model:** also walk the **LLM Top 10** — prompt injection (untrusted
   ...
   in `{{skill_file:skill://security-and-hardening}}` → *LLM / AI Feature Security* (opt-in; state any
...
unanalyzed; secrets/PII handling explicitly addressed (`rule://secrets-policy` /
