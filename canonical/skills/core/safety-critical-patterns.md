...
> rules, via the MIT-licensed [`athola/{{external_literal:external-skill-collection}}`](https://github.com/athola/{{external_literal:external-skill-collection}})
...
uniformly. Use `rule://risk-classification` to place the change:
...
- `rule://risk-classification` — sets the consequence tier that gates how much of this to apply
...
- `rule://testing` — assertions complement, never replace, the runnable test for non-trivial logic
