...
Companion rules: `rule://code-organization` (auth & dependency patterns), `rule://design-patterns` (service/repository boundaries), `rule://documentation` (security documentation). The `security-reviewer` agent (Phase 5.4 in `rule://mandatory-workflow`) enforces what this skill teaches.
...
> Distinct from `rule://agent-guardrails`, which secures the *{{host:product}} agent itself*. This
...
See `artifact://skill-reference-security-checklist` for the full pre-commit checklist.
