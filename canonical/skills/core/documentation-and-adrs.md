...
- **Regenerate + drift-check in CI.** The generator should have a `--check` mode that fails CI when
  ...
  a guarantee. ({{kit:cli}} itself lives this rule: its hook config files are generated from a
...
- **{{ref:artifact://project-instructions}} / rules files** — Document project conventions so agents follow them
...
> [`athola/{{external_literal:external-skill-collection}}`](https://github.com/athola/{{external_literal:external-skill-collection}}) `slop-detector` /
...
- [ ] Rules files ({{ref:artifact://project-instructions}} etc.) are current and accurate
