...
request_input:
  mode: optional
  hint: '[API endpoint or feature name]'
pause_for_human: []
references:
- skill://security-and-hardening
---

Integrate API for: {{request}}.

## Steps

1. **Identify the API**: Determine the endpoint(s), HTTP method(s), request/response shapes, and authentication requirements for {{request}}.
...
- **Redact secrets at record time.** Strip `Authorization`, API keys, cookies, and any PII from the
  ...
  credentials (ties to `{{skill_path:skill://security-and-hardening}}` and `secret-scanner`).
