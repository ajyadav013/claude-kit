...
- Tracking model performance, latency, and cost across {{external_assistant:title}}, OpenAI, or other providers
...
**Provider and model labeling**: Always include `provider` (anthropic-vertex, openai, {{external_assistant:lower}}, etc.) and `model` ({{external_literal:external-model-current-balanced}}, {{external_literal:external-model-multimodal}}, etc.) as metadata to enable per-provider/model filtering in Langfuse UI.
...
def generate_summary(text: str) -> str:
    with trace_completion(
        ...
        model="{{external_literal:external-model-current-balanced}}",
...
async function generateCode(prompt: string, userId: string): Promise<string> {
  ...
  const response = await langfuseService.traceOpenAICall(
    'generate_code',
    {
      model: '{{external_literal:external-model-multimodal}}',
      messages: [{role: 'user', content: prompt}],
      max_tokens: 2000,
    },
