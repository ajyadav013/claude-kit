...
`rule://devops-observability` and the `observability-engineer` define. Most cliffs are
...
1. **Never load-test production** without explicit approval — test a staging/local stack. See `rule://human-in-the-loop`.
...
> Adapted from a portfolio project's load-testing skill; generalized to be stack- and tool-agnostic for {{kit:cli}}.
