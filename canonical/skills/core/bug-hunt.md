...
`rule://quality-gates`); do not invent a parallel scale. Calibrate by blast radius and
likelihood:
...
- Any area you could not fully assess (so the gap is visible, not silently dropped — see
  `rule://quality-gates` §2.5 on evidence).
...
- Report what you did **not** cover. A bounded hunt that names its blind spots beats one that
  implies total coverage. See `rule://agent-guardrails` §2.
