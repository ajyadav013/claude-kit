...
When the pip CLI is installed, `{{kit:cli}} tickets` renders the store as a live chart — one row per
...
the commit graph with each commit's ticket attached, `{{kit:cli}} tickets <PREFIX>-<N>` opens one
ticket's detail with its full work log, and `--watch 5` re-renders while a run is in flight.

For a browser view, `{{kit:cli}} tickets --html` writes a Kanban board to
`state://ticket-board` and prints a `file://` URL. It is a plain file, not a server — the
...
- **The board opens once, here.** `{{kit:cli}} tickets --open` at ticket creation gives the human a
