...
pause_for_human: []
references:
- artifact://change-proposal-template
- state://workflow
---
...
- **Cite evidence, never paste it** — executed output (test runs, measurements, gate proofs) lives at its `state://workflow` or artifact path; the spec links to it, so it stays requirements-sized for every agent that reads it.
