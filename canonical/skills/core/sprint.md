...
Generate a sprint plan for backlog item #{{request}}.
...
context-budget guidance in `rule://agent-resilience`):
...
- **Hand off cleanly on replacement.** Before retiring an agent, capture its state to working memory
  (`state://continuity`) and check `git status` so nothing in-flight is lost; the replacement
  resumes from that snapshot (reload, don't re-run — see `rule://continuity`).
