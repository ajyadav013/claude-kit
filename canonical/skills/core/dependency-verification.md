...
> [`athola/{{external_literal:external-skill-collection}}`](https://github.com/athola/{{external_literal:external-skill-collection}}) `dependency-verification` skill.
...
*author-shipped, version-synced* skills into `artifact://skill-library/` (managed symlinks; `uvx library-skills`
...
(`rule://agent-guardrails` §4). This section is the **canonical intake check** for any
...
| **Privilege-carrier abuse** | Frontmatter grants (`{{provider_metadata:tool-allowlist}}`, a broad `tools:` list) far wider than the artifact's stated job needs. |
...
### Intake procedure (before the artifact enters `state://root/` or `artifact://mcp-configuration`)
...
5. **NEVER "load it to inspect it".** A stdio MCP entry **executes its command the moment a client
   loads the config** — adding an untrusted server to `artifact://mcp-configuration` "to see what tools it has" *is*
...
- [ ] Any third-party agent-config artifact passed the intake procedure (provenance pin · structural read · hidden-content scan · credential-routing test) before entering `state://root/` or `artifact://mcp-configuration`, and no untrusted MCP entry was loaded "to inspect it"
