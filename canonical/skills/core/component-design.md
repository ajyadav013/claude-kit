...
request_input:
  mode: optional
  hint: '[component name and purpose]'
pause_for_human: []
references:
...
Design and implement the component: {{request}}.

## Steps

1. **Read the design system**: Read `rule://ui-design-system` (foundations & index) and `rule://ui-components` (component standards — cards, badges, buttons, form controls, compound components), both installed as React overlay rules — or a project's `docs/references/ui/ui-design-system.md` equivalent. Read whichever is present.
...
- Design system: `rule://ui-design-system` (foundations) + `rule://ui-components` + `rule://ui-layout-and-motion` (React overlay) or `docs/references/ui/ui-design-system.md` (if present)
- UX patterns: `rule://ux-patterns` + `rule://ux-dashboard-patterns` (React overlay) or `docs/references/ui/ux-patterns.md` (if present)
- Rules: `rule://frontend-best-practices`, `rule://responsive-and-accessibility`
