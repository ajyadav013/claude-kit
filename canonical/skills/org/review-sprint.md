...
1. **Deliverable inventory — for each planned deliverable:**
   - **Exists** — the file/endpoint/component is actually present (verify with {{tool:file_glob}}/{{tool:file_search}}, don't trust
     the report).
...
2. **Acceptance criteria.** Check each acceptance criterion explicitly — met, not just claimed. A
   "green" claim must cite the real check + output (`rule://quality-gates` §2.5).
...
(`rule://quality-gates`); not clear with any Critical/High/Medium open.

## Stop conditions
Stop and escalate (`rule://human-in-the-loop`) if the sprint plan or scope doc is missing
