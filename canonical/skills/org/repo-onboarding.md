...
5. Should findings be **captured as docs** (run
   `{{skill_invocation:skill://documentation-and-adrs}}`) or just explained?

## Agents to delegate to
A read-only research role (map structure, entry points, modules, conventions, the project's
linter / test runner / build commands). Then, if capturing:
`{{skill_invocation:skill://documentation-and-adrs}}` to write an onboarding/architecture doc and
`{{skill_invocation:skill://refresh-docs}}` to keep it in sync. No implementation agents — this
...
Optionally a captured onboarding doc via
`{{skill_invocation:skill://documentation-and-adrs}}`.
...
## Example
```
{{skill_invocation:skill://repo-onboarding}} I'm a new engineer — how does this service fit together and where do I start?
→ asks: what will you work on first? overview or deep dive? capture as docs?
→ a read-only research pass maps structure, entry points, modules, conventions, build/test commands
→ produces "where things live" + an ordered onboarding path (read → run → first small task)
→ optionally captures it via {{skill_invocation:skill://documentation-and-adrs}}, kept in sync with {{skill_invocation:skill://refresh-docs}}
```
