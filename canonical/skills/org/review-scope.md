...
**main gap**. Rate findings **Critical / High / Medium / Low / Cosmetic** (`rule://quality-gates`);
not clear with any Critical/High/Medium open.

## Stop conditions
Stop and escalate (`rule://human-in-the-loop`) if user intent is genuinely ambiguous, if the
