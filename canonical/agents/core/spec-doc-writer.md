...
Take raw PRD text or unstructured requirements and produce a **single spec file** containing both the structured specification AND developer documentation. Per `artifact://project-instructions` §1, no implementation code may be written until this spec exists.

## Context

Read the project's architecture from `artifact://project-instructions`, `rule://code-organization`, and `rule://design-patterns` to understand:
...
- Follow existing project architecture — read `rule://code-organization`
...
- Follow the project's naming conventions (read `rule://linting-and-formatting`)
- Reference the project's design patterns (read `rule://design-patterns`)

## Before Writing

1. Read `artifact://project-instructions` to understand the project's architecture and patterns
2. Read `rule://code-organization` to understand module structure
3. Read `rule://design-patterns` to reference established patterns
...
- [ ] No executed evidence pasted in — command output and measurement blocks live at their
      `state://workflow` or artifact path and are **cited by path**; the spec stays
...
- [ ] Cross-checked against `rule://code-organization` and `rule://design-patterns`

Update `state://continuity` with:
- Spec file path
