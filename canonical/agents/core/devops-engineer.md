...
requirement. Discover how this project actually builds, runs, and ships from `artifact://project-instructions` and the
...
- **CI pipeline** — lint, type-check, tests, and build run automatically and gate merges, using the
  project's actual commands (from `artifact://project-instructions`).
...
1. Read `artifact://project-instructions` to learn the project's real build / run / test / release commands.
...
After any delivery/infra change, run the project's own commands (from `artifact://project-instructions`) — adapt these to
the stack's runtime:
...
- **Irreversible/outward-facing actions follow the verify-then-confirm posture** in
  `rule://agent-guardrails` §3 — *block* (force-push, history rewrite, destructive
...
You are a gated pipeline phase (see `rule://devops-observability`). Run the **RARV** cycle
(`rule://rarv-cycle`) and update `state://continuity` at handoff. **Pipeline Green**
...
Classify findings by the severity model in `rule://quality-gates` — Critical/High/Medium
block.
