...
- `agent://senior-frontend-reviewer` when a frontend surface exists.
- `agent://senior-backend-reviewer` when a backend surface exists.
- `agent://technical-architect` for cross-system interfaces, boundaries, and non-functional invariants.
- `agent://devils-advocate` in standard/enterprise when risk or uncertainty is present.

Every reviewer returns `PASS | FAIL` and the stable finding fields required by
`rule://quality-gates` §3. It may block only on an evidenced Critical/High/Medium defect. Preferences,
...
- **Spawn once**: `agent://em-reviewer` with the frozen generation and all verdicts.
- The EM de-duplicates findings by violated criterion/invariant plus evidence, applies the authority
  table in `rule://quality-gates`, and issues one consolidated `PASS | FAIL` decision. There is no
  vote and no reviewer-to-reviewer reply chain.
- On FAIL, route one consolidated register to `agent://spec-doc-writer` / `agent://ui-designer`.
...
| 3a-FE | `agent://senior-frontend-reviewer` | Frontend feasibility on frozen plan | Yes — blind panel |
| 3a-BE | `agent://senior-backend-reviewer` | Backend feasibility on frozen plan | Yes — blind panel |
| 3b | `agent://technical-architect` | Cross-system architecture on frozen plan | Yes — blind panel |
| PC | `agent://devils-advocate` | Conditional adversarial plan challenge | Yes — blind panel |
| 3c/JOIN | `agent://em-reviewer` | De-duplicates and adjudicates one panel result | No — gate |
