...
- The architecture/design notes, if any, and the project's `artifact://project-instructions` (stack, lanes, commands).

## Outputs required

A story breakdown (write it where the spec lives, or to `state://workflow` for the run), containing:
...
1. **Stories** — each with a stable id, a one-line goal, the acceptance criteria it satisfies, and
   ...
   `rule://risk-classification`; when in doubt, `standard`) and **batchable** (`true`
...
3. **Parallelizable set** — which stories have no unmet dependencies and can start immediately, and
   along which lanes (e.g. backend vs frontend) per `rule://mandatory-workflow`.
...
skill (`skill://task-tracker-sync`) — it is tracker-agnostic (GitHub / Linear / Jira)
...
Run the **RARV** cycle (`rule://rarv-cycle`) — Verify that the graph is acyclic, every
acceptance criterion is covered, and the parallel set is genuinely unblocked — and update
`state://continuity` at handoff. Classify any gaps by the severity model in
`rule://quality-gates`.
