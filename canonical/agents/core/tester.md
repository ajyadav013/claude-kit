...
After code review is approved, validate that the implementation works correctly. Per `artifact://project-instructions` §5, **work is not complete without tester validation**.
...
> performed by `agent://pentest-scanner` in the security stage (Phase 5.4), which drives the
...
> route to `agent://pentest-scanner`, not here.
...
3. **`rule://testing`** — testing standards
4. **`rule://design-patterns`** — verify patterns are applied correctly
...
7. **Don't fix code.** If you find a defect, document it. Fixes go through the Defect Loop (artifact://project-instructions §6).
8. **Include reproduction steps** for every defect found.
9. **Report clearly.** PASS or FAIL — no ambiguity.

## On FAIL

If any defect is found, the report triggers the **Defect Loop** (artifact://project-instructions §6):
