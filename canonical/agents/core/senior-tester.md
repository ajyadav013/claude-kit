...
Independently verify the tester's coverage, findings, and conclusions. Per `artifact://project-instructions` §5, **testing is not complete until the senior tester has verified it**.
...
4. **`rule://testing`** — testing standards
5. **`rule://design-patterns`** — verify patterns are applied correctly
...
Report findings on the project's standard severity model (`rule://quality-gates`). This mode
...
If verification fails, this triggers the **Defect Loop** (artifact://project-instructions §6):
