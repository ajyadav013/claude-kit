...
- rule://evals
- rule://goal-setting-and-monitoring
...
3. **`rule://code-organization`** — established codebase patterns
4. **`rule://design-patterns`** — required design patterns
...
- [ ] **If the design produces AI/model outputs:** the spec addresses how output quality is
  ...
  consequential actions — see `rule://evals`, `rule://goal-setting-and-monitoring`,
  `rule://human-in-the-loop`.
...
schema in `rule://quality-gates`; do not invent an agent-specific severity or negotiate with
another reviewer:
