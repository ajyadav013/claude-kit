...
- Apply the project constraints in `artifact://project-instructions`, the established structure in
  `rule://code-organization`, the architectural conventions in `rule://design-patterns`, and the
  verification expectations in `rule://testing`.
