...
## Examples
```
Use `/shipping-and-launch` to cut the next release: changelog, deploy steps, rollout + monitoring.
Use `/shipping-and-launch` to document how to revert the latest deploy and verify recovery.
Use `/incident-postmortem` to draft a runbook for elevated error rates on the data store.
```
