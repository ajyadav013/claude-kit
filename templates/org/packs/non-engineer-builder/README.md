...
| Learn what a repo does before changing it | `/repo-onboarding` → a read-only research role |
...
## Examples
```
Use `/prototype-to-production` to turn an onboarding-checklist prototype into scoped, tested, approval-gated work.
Use `/customer-issue-to-fix` to reproduce and safely route a broken export-button report.
Use `/feature-from-idea` to turn a bulk-tagging idea into a clarified brief and scoped task.
```
